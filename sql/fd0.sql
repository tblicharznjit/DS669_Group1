DROP TABLE IF EXISTS final_dataset2;
...
bg.ph_avg as PH,
bg.po2_avg as PaO2,
bg.pco2_avg as PaCO2,
bg.pao2fio2_avg as `PaO2/FiO2`,
bg.baseexcess_avg as ArterialBE,
bg.bicarbonate_avg as HCO3,
(bg.fio2_avg)/100 as FiO2,
bg.hemoglobin_avg as HGB,
bg.chloride_avg as Chloride,
bg.calcium_avg as Calcium,
bg.magnesium_avg as Magnesium,
bg.sgpt_avg as SGPT,
bg.sgot_avg as SGOT,
...
LEFT JOIN `temp_bg_combined` bg ON bg.subject_id = b.subject_id AND bg.hadm_id = b.hadm_id AND bg.icustay_id = b.icustay_id AND bg.hour_offset = b.hour_offset
LEFT JOIN `vitals_4bin` v ON v.subject_id = b.subject_id AND v.hadm_id = b.hadm_id AND v.icustay_id = b.icustay_id AND v.hour_offset = b.hour_offset
LEFT JOIN `labVals_4bin` l ON l.subject_id = b.subject_id AND l.hadm_id = b.hadm_id AND l.icustay_id = b.icustay_id AND l.hour_offset = b.hour_offset
LEFT JOIN `bilirubin_4bin_total` bi ON bi.subject_id = b.subject_id AND bi.hadm_id = b.hadm_id AND bi.icustay_id = b.icustay_id AND bi.hour_offset = b.hour_offset
